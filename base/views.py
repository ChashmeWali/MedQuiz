...
def search_users_view(request):
    ...
    if query:
        ...
        # Fetch profile images for the found users
        user_profiles = Profile.objects.filter(user__in=users)
        user_profile_dict = {profile.user_id: profile.profile_images.url for profile in user_profiles}
    ...
    if request.user.is_authenticated:
        ...
        user_object = User.objects.get(username=request.user)
        user_profile = Profile.objects.get(user=user_object)
        context = {"user_profile": user_profile, "query": query, "users": users, "user_profile_dict": user_profile_dict}
    else:
        context = {"query": query, "users": users, "user_profile_dict": user_profile_dict}
